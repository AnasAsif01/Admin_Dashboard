...
$(document).ready(function () { 
  ...
  else if(role==1)
  {
    ...
    $.ajax({
      type: "POST",
      url: "../backend/getnewchat.php",
      data:{vendorid:vendorid},
      cache: false,
      success: function(data) {
        const parsedData = JSON.parse(data);
        const products = parsedData.data;
        count = count + products.length-1;
        document.getElementById('notificationcount2').textContent = count;
        const dropdownMenu = $('.usermsg');
        dropdownMenu.append('<h6 class="dropdown-header">Alert Center</h6>');
        products.forEach(function(product) {
          const name = product.Name;
          const html = '<a class="dropdown-item d-flex align-items-center" href="#">' +
            '<div class="me-3">' +
            ...
            '</div>' +
            '<div>' +
            `<p>${name} Has send you a message</p>` +
            '</div>' +
            '</a>';
          dropdownMenu.append(html);
        });
